...
# Блок обработки вирутальных машин
	...
	if [[ -z "${vmhost_id}${device_id}" ]] ; then
		log 4 "${hst} Help me! Who am I? VM or Device?"
	else
		...
		for i in "${!IFState[@]}" ; do
			if [[ "${i}" =~ ^(eth|eno|enp|ens|br|p|P)[0-9][0-9]*[sfduci0-9]* ]] ; then
				if [[ ! $(echo " ${jiface_names} " | grep "${i}" ) ]] ; then
					log 3 "${hst}Create interface $i - mtu ${IFMtu[$i]} mac ${IFMac[$i]} type ${IFType[$i]}" 
					if [[ -z "${vmhost_id}" ]] ; then 
